...
/**
 * Created by slack on 2020/6/3 下午10:22.
 */
public class MicRecorder implements Encoder {
    ...
    private static class CallbackDelegate extends Handler {
        ...
        CallbackDelegate(Looper l, EncoderCallback callback) {
            super(l);
            this.mCallback = callback;
        }
    ...
    private void feedAudioEncoder(int index) {
        ...
        int flags = android.media.MediaCodec.BUFFER_FLAG_KEY_FRAME;

        if (eos) {
            flags = android.media.MediaCodec.BUFFER_FLAG_END_OF_STREAM;
        }
